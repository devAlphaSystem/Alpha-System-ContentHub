document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderTableRow(footer) {
    ...
    const editUrl = `/projects/${projectId}/changelog_footers/edit/${escapeHtml(footer.id)}`;
    const deleteUrl = `/projects/${projectId}/changelog_footers/delete/${escapeHtml(footer.id)}`;
    ...
    return `
      <tr data-footer-id="${escapeHtml(footer.id)}" data-updated-timestamp="${updatedTimestamp}">
        ...
        <td data-label="Actions" class="actions-cell">
          <a href="${editUrl}" class="btn btn-icon btn-edit" title="Edit Changelog Footer"><i class="fas fa-pencil-alt"></i></a>
          <form action="${deleteUrl}" method="POST" class="delete-cl-footer-form" title="Delete Changelog Footer">
  ...
  function renderTable(footers) {
    ...
    if (footers.length > 0) {
      ...
      footersTableBody.innerHTML = `<tr class="no-match-row"><td colspan="${colSpan}" style="text-align: center; padding: 20px; color: var(--text-muted);">No changelog footers found for this page.</td></tr>`;
      if (dataCard) dataCard.classList.remove("hidden");
      if (emptyStateCard) emptyStateCard.style.display = "none";
    } else {
  ...
  async function fetchFooters() {
    if (isLoading || !projectId) {
      if (!projectId) console.warn("Project ID missing, cannot fetch changelog footers.");
      return;
    }
    isLoading = true;
    ...
    const url = `/api/projects/${projectId}/changelog_footers?page=${currentPage}&perPage=${itemsPerPage}&sort=${sortParam}`;
    ...
    try {
      ...
      console.error("Failed to fetch changelog footers:", error);
      window.showAlertModal("Error loading changelog footers. Please try refreshing.", "Loading Error");
  ...
  function handleFormSubmit(event) {
    ...
    window.showConfirmModal({
      ...
      message: `Are you sure you want to delete the changelog footer "<strong>${escapeHtml(footerName)}</strong>"?`,
      action: "delete",
      confirmText: "Delete",
    });
  }
  ...
  function initializeClFooters() {
    if (!projectId) {
      console.warn("Project ID missing, cannot initialize changelog footers.");
      return;
    }
