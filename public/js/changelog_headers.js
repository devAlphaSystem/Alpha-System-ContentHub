document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderTableRow(header) {
    ...
    const editUrl = `/projects/${projectId}/changelog_headers/edit/${escapeHtml(header.id)}`;
    const deleteUrl = `/projects/${projectId}/changelog_headers/delete/${escapeHtml(header.id)}`;
    ...
    return `
      <tr data-header-id="${escapeHtml(header.id)}" data-updated-timestamp="${updatedTimestamp}">
        ...
        <td data-label="Actions" class="actions-cell">
          <a href="${editUrl}" class="btn btn-icon btn-edit" title="Edit Changelog Header"><i class="fas fa-pencil-alt"></i></a>
          <form action="${deleteUrl}" method="POST" class="delete-cl-header-form" title="Delete Changelog Header">
  ...
  function renderTable(headers) {
    ...
    if (headers.length > 0) {
      ...
      headersTableBody.innerHTML = `<tr class="no-match-row"><td colspan="${colSpan}" style="text-align: center; padding: 20px; color: var(--text-muted);">No changelog headers found for this page.</td></tr>`;
      if (dataCard) dataCard.classList.remove("hidden");
      if (emptyStateCard) emptyStateCard.style.display = "none";
    } else {
  ...
  async function fetchHeaders() {
    if (isLoading || !projectId) {
      if (!projectId) console.warn("Project ID missing, cannot fetch changelog headers.");
      return;
    }
    isLoading = true;
    ...
    const url = `/api/projects/${projectId}/changelog_headers?page=${currentPage}&perPage=${itemsPerPage}&sort=${sortParam}`;
    ...
    try {
      ...
      console.error("Failed to fetch changelog headers:", error);
      window.showAlertModal("Error loading changelog headers. Please try refreshing.", "Loading Error");
  ...
  function handleFormSubmit(event) {
    ...
    window.showConfirmModal({
      ...
      message: `Are you sure you want to delete the changelog header "<strong>${escapeHtml(headerName)}</strong>"?`,
      action: "delete",
      confirmText: "Delete",
    });
  }
  ...
  function initializeClHeaders() {
    if (!projectId) {
      console.warn("Project ID missing, cannot initialize changelog headers.");
      return;
    }
